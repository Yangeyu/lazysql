...
- **词法级 pretty-printer**(`prettyJson`):只在 token 之间重排空白,token 原样搬运——`JSON.parse→stringify` 会写坏 2^53 以上的大数、解掉转义,故弃用;view 面同步换用。**端到端边界**:PG `jsonb` / MySQL `json` 由驱动(Bun.SQL / mysql2)先解析成 JS 对象、`normalizeCell` 再 stringify,大数在进 UI 之前就已按 JS number 归一——formatter 的保真只对**以字符串抵达的 JSON**(TEXT 列、SQLite)端到端成立;根治需驱动层 json-as-string(后续项)。
