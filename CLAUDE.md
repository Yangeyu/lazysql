...
- **注释**：解释「为什么」，不解释「做了什么」——能靠**改名/类型**说清的就别注释。默认偏少（删了它会丢失"读代码无法恢复"的信息吗？不会就删）。
  - **留**：取舍理由（为什么不选那个显然方案）· 类型表达不了的非局部不变量/陷阱 · 指向 `adr/00xx`·`ARCHITECTURE §x` 的指针 · 公共契约的 doc 注释。
  - **禁**：复述代码 · 注释掉的代码（删，git 记得）· 变更历史/署名（`// added by…`）· 旁白 diff（`// 现在改为…`）· 补偿烂名字（改名）。
  - **文件头**：非平凡模块开头照 `DataSource.ts` 的形状，讲「是什么 / 为什么这样设计 / 指针」。注释用**英文**（与代码一致，勿中英混注）；`// ── 分节线 ──` 这类轻量结构标记可留。
