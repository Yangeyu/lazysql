...
`TypeScript(strict)` · `Bun` · `OpenTUI`(`@opentui/react` + React 19)· `Zustand` · `yaml`
LLM:`@anthropic-ai/sdk` + 任意 OpenAI 兼容后端(Qwen / OpenAI / DeepSeek)
...
```bash
bun start                 # 打开默认(第一个已保存的)连接
bun start <name>          # 按 id / 名称打开已保存的连接
...
```bash
export DASHSCOPE_API_KEY=sk-xxx && bun start          # 默认 Qwen
ANTHROPIC_API_KEY=sk-ant-xxx LAZYSQL_LLM_PROVIDER=anthropic bun start  # 临时切 Claude
```
