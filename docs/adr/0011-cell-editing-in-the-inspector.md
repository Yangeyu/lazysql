...
## 补记（2026-07-03）：边界 3 落地——JSON 编辑格式化

「按列类型美化编辑」落地,原始值原则不变、只在**可证明无损**处放开:

- **词法级 pretty-printer**(`prettyJson`):只在 token 之间重排空白,token 原样搬运——`JSON.parse→stringify` 会写坏 2^53 以上的大数、解掉转义,故弃用;view 面同步换用。**端到端边界**:PG `jsonb` / MySQL `json` 由驱动(postgres.js / mysql2)先解析成 JS 对象、`normalizeCell` 再 stringify,大数在进 UI 之前就已按 JS number 归一——formatter 的保真只对**以字符串抵达的 JSON**(TEXT 列、SQLite)端到端成立;根治需驱动层 json-as-string(后续项)。
- **`ColumnDef.jsonCanonical`**(adapter 声明,「存储不保留 JSON 文本格式」):PG `jsonb`、MySQL `json` 标记之;PG `json` 保留原文**刻意不标**,Mongo 写回是字符串原样存(`patchToDoc` 不解析)也不标。标记列的编辑种子自动美化、保存前先验 JSON、与种子全等的保存视为 no-op 不落 UPDATE。列类型来自打开对象时已缓存的 describe(`structure`);自由查询无此元数据 → 维持原始值编辑。
- **手动格式化(`^R`)曾实现后移除**:非白名单来源(text 列 / SQLite / 自由查询)本可显式按键重排,从简砍掉——它需要一条「命令经 store 状态传给 widget」的信号通道(`formatSeq`),复杂度与收益不配。若将来需要,先补 store 侧草稿镜像(照 ADR 0010 §2 的 reconcile 形状)再做,不走信号计数器。
