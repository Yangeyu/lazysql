...
把「单连接即重建 store + 独立 picker 屏」改为：**一个长生命 store 作为工作台**，活动 `DataSource` 被换入换出。
- `source: DataSource | null`——无活动连接时 store 仍在，侧栏显示连接根列表（`○`），`n` 仍可用。
- 连接根行：活动者（`●`）折叠展开其 schema；非活动者（`○`）`Enter` 即连接（切换）。
- **store 绝不碰 driver/repo/secret**：它只通过注入的 `Workbench`（`connect`/`saveConnection`/`removeConnection`）
  发出意图，由 `Root`（组合根）执行基础设施动作——DIP 不破。`Root` 持有 repo/secrets/factory，
  把 picker 折叠进侧栏，按需把新源 `makeStore` 进工作台。
- `n` 表单（`ConnectionForm`，store `mode:'connform'`）：选 driver + 填字段 → 经 `ConnectionRepository.save`
...
3. **`Root` 闭包**：工作台用 ref + 稳定 `Workbench` 桥接「重建的 store」与「最新的 profiles/activeId」，
   是为隔离基础设施而付的一点 React 编排复杂度。
