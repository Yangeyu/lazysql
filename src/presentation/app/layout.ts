...
// The panes sit FLUSH (no gutter), so the grid's top border begins on the row
// immediately below the editor pane. Centralizing these here is the whole point
// of this module: the mouse and the renderer derive row positions from the SAME
...
export const hitTest = (l: HitLayout, x: number, y: number): Hit | null => {
  ...
  // Right side: the editor pane occupies the first `editorRows` body rows; the
  // grid sits flush below it (no gap row).
