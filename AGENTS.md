...
5. **死代码**：`tsconfig` 开 `noUnusedLocals` + `noUnusedParameters` —— 未使用的导入/局部量/参数直接 typecheck 红。
6. **DONE 门禁**：`.githooks/pre-push` 跑 `bun run typecheck && bun test`。
7. **commitlint**：`.githooks/commit-msg`（见 `docs/commit-convention.md`）。
